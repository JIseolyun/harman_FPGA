...
module top_watch (
    input clk,
    // Basys3 5개 버튼
    input btnC,  // 중앙: 모든 모드(둘 다)에서 reset
    ...
    // (시계는 자동으로 1초마다 sec가 증가하고, 60sec→min, 60min→hour로 연결되도록 clock_dp 내부에서 처리)
    ...
    // sw_clock = 1이면 시계, sw_clock = 0이면 스톱워치 출력을 사용
    ...
    // 6) LED 모드 표시 (4개)
    // LED[0]: 시계 (시간:분), LED[1]: 시계 (초:밀리초),
    // LED[2]: 스톱워치 (시간:분), LED[3]: 스톱워치 (초:밀리초)
